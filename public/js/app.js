...
async function loadControl() {
  ...
  $('#controlPanel').innerHTML = `
    <div class="stat-grid"> <div class="stat"><div class="num">${ai.mode === 'ai' ? 'AI' : 'Demo'}</div><div class="lbl">Categorisatie-modus</div></div> <div class="stat"><div class="num">${ai.handled}</div><div class="lbl">Berichten verwerkt</div></div> <div class="stat"><div class="num">${ai.accuracy === null ? '—' : ai.accuracy + '%'}</div><div class="lbl">Juist ingedeeld (na controle)</div></div> <div class="stat"><div class="num">${ai.corrected}</div><div class="lbl">Door mens gecorrigeerd</div></div> <div class="stat"><div class="num">${stats.pendingReviews}</div><div class="lbl">Wacht op controle</div></div> </div> <div class="info-card" style="max-width:680px"> <h3>Automatisch goedkeuren (zonder controle)</h3> <p class="muted small">Hoe zeker moet de AI zijn voordat een aanvraag <strong>vanzelf</strong> een opdracht wordt en naar de monteur gaat — zonder dat iemand kijkt? Standaard geldt dit voor opdracht-groepen (DRS) en het eigen websiteformulier. Met het vinkje hieronder ook voor losse e-mails en 1-op-1 WhatsApp-berichten. Nooit voor reacties op onze eigen mails, leveranciers- of reclamemail, of aanvragen zonder contactgegevens. Zet op 0% om alles handmatig te controleren. <em>De knop "Accepteer boven drempel" in de Inbox keurt eenmalig de lijst goed die je op dat moment ziet.</em></p> <label>Vanzelf goedkeuren vanaf: <strong id="threshLbl">${pct}%</strong> <input type="range" id="threshold" min="0" max="100" step="5" value="${pct}"></label> <label style="display:flex;align-items:center;gap:8px;flex-direction:row;margin:8px 0"><input type="checkbox" id="autoAllChannels" style="width:auto" ${settings.autoApproveAllChannels ? 'checked' : ''}> Ook losse e-mails en 1-op-1 WhatsApp-berichten automatisch goedkeuren boven de drempel</label> <button class="btn btn-primary" id="saveThreshold">Opslaan</button> <div class="muted small" style="margin-top:8px">${pct ? `Nu: aanvragen met minimaal ${pct}% AI-zekerheid worden vanzelf een opdracht${settings.autoApproveAllChannels ? ' — via alle kanalen' : ' — alleen uit opdracht-groepen en het websiteformulier'}.` : 'Nu: 0% — alles wordt handmatig gecontroleerd.'}</div> </div> ${ai.mode === 'demo' ? '<p class="muted small" style="max-width:680px;margin-top:14px">De AI draait nu in <strong>demo-modus</strong> (regels). Vul een Claude API-sleutel in (<code>ANTHROPIC_API_KEY</code>) voor slimmere categorisatie. Zie docs/INTEGRATIES.md.</p>' : '<p class="muted small" style="margin-top:14px">Slimme AI (Claude) is actief.</p>'}
  ...
  $('#saveThreshold').onclick = async () => {
    try {
      await api('/api/settings', 'PATCH', { aiAutoApproveThreshold: Number(range.value) / 100, autoApproveAllChannels: !!$('#autoAllChannels')?.checked });
      toast(Number(range.value) ? `Opgeslagen: vanaf ${range.value}% vanzelf goedkeuren${$('#autoAllChannels')?.checked ? ', via alle kanalen' : ''}` : 'Opgeslagen: alles handmatig (0%)');
      loadControl();
    } catch (err) { toast(err.message, true); }
  };
